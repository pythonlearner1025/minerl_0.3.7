...
class SmeltWorker(CraftWorker):
    ...
    def smelting_once(self, target: str,  recipe_info: Dict, target_num, fuels):
        ...
        if fuels!='coals':
            ...
            if inventory_num > 1:
                self.pull_item_return(slot_pos, inventory_id,fuels)
            for _ in range(int(cook_time*target_num-10)):
                reserve = random.choices([False,True],weights=[0.975,0.025],k=1)[0]
                self._null_action(1, reserve=reserve)
            for _ in range(10):
                self._null_action(1, reserve=True)
